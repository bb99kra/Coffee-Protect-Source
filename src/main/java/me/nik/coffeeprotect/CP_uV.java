 ...
 */
public abstract class CP_uV {
    protected final CP_TM CP_w;
    ...
    public CP_uV(CP_TM ilIlTM, boolean bl) {
        long l = a ^ 0x5FBDCF55BD59L;
        this.CP_w = ilIlTM;
        this.CP_H = bl;
        CP_jl ilIljl = this.getClass().getAnnotation(CP_jl.class);
        try {
            if (ilIljl == null) {
                Bukkit.getPluginManager().disablePlugin((Plugin)CoffeeProtect.CP_J());
                throw new CP_fF(c + this.getClass().getSimpleName() + ".");
            }
        }
        catch (CP_fF ilIlfF) {
            throw CP_uV.a(ilIlfF);
        }
        this.CP_U = ilIljl.CP_J();
        this.CP_I = ilIljl.CP_M();
    }

    public abstract CP_TV CP_h(PacketReceiveEvent var1);
    ...
    private static CP_fF a(CP_fF ilIlfF) {
        return ilIlfF;
    }
